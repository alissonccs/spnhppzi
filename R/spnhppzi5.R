...
spnhppzi5<-function(formula,
  ...
  time <- as.numeric(resp[,1])
  event <- as.numeric(resp[,2])
  id <- as.numeric(resp[,3])
  SP_ID<-resp[,4]
  IndRec<- as.numeric(resp[,5])
  ...
  max_stop <- as.numeric(as.vector(unlist(data1[,2])))
  IndRec2<- as.numeric(as.vector(unlist(data1[,4])))
