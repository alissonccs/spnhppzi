...
simrecev <- function(N,
  ...
  U <- runif(N)
  Y <- (-1) * log(U) * exp((-1) * x %*% beta.x) * 1 / z #tempo de sobrevivência veja artigo Generating survival times to simulate pag 1717 tabela II
  if (dist.rec == "lognormal") { # lognormal
    t <- exp(qnorm(1 - exp((-1) * Y)) * sigma + mu)
  } else if (dist.rec == "weibull") { # weibull
    t <- ((lambda)^(-1) * Y)^(1 / nu) #Inverso da função acumulada
  ...
  # print(T)
  dirty <- rep(TRUE, N)
  pdt <- t(rbinom(N, 1, pfree1))
  dirty <- as.vector(ifelse(pdt==1, !dirty, dirty))
  # print(dirty)
  ...
  while (any(dirty)) {
    ...
    ######
    if(logist==1){
      pfree<-1/(1+exp(-(1+x %*% beta.x)))
    }
    # print(pfree)
    #####
    pd <- rbinom(N, 1, pfree1)
    # pdt<-t(pd)
    U <- runif(N)
    ...
    # print((pdt))
    # dirty <- cbind(dirty, ifelse(pd==1, NA, dirty))
    # print((dirty))
    if (dist.rec == "lognormal") { # lognormal
    ...
    #print(dirty)
    T1 <- cbind(T1, ifelse(dirty|i==1, t1, NA))
    # print(T1)
